...
def main() -> None:
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        if args.workers <= 1:
            for cfg in configs:
                try:
                    _write_row(writer, _serialise_run(cfg))
            ...
            try:
                ...
                print(f"  Process pool unavailable ({e}); falling back to sequential execution.",
                      file=sys.stderr)
                for cfg in configs[n_done:]:
                    try:
                        _write_row(writer, _serialise_run(cfg))
                    except Exception as run_error:
                        n_failed += 1
                        print(f"  FAILED {cfg.as_tag()}: {run_error}", file=sys.stderr)
                    n_done += 1
                    if n_done % args.progress_every == 0 or n_done == n_total:
                        mark_progress()
