"""
Cluster-robust re-analysis for AAAI reviewer points #3, #4, #8. Real data, deterministic.
"""
...
B = 4000
peer = pd.read_csv("tmp/paper1_clean_experiments_full/multiagent_cadence_full/runs.csv")
...
print(f"peer rows={len(peer)}  independent design cells={len(cells)}  seeds={peer.seed.nunique()}")
print(f"Naive n=25,920 but only {len(cells)} independent config cells; block bootstrap resamples cells.\n")
