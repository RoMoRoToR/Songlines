...
\textbf{Two environments.} Single-agent runs on MiniGrid/BabyAI; multi-agent on a custom $12{\times}10$ grid. Identical operational $Q^\star,R^\star,M^\star,C^\star$ definitions recover interpretable stage profiles on both substrates, with the same chain ordering and weakest-stage attribution. A multi-agent MiniGrid wrapper unifying the two is the next portability step (Appendix~\ref{app:open-problems}, item 4).
...
\textbf{(4) Substrate generality.} The decomposition transfers across MiniGrid, BabyAI, and the custom grid with identical operational definitions; a multi-agent MiniGrid wrapper and a continuous-state environment are the next portability steps.
