...
```
papers/                      # All six manuscripts (EN + RU), one folder per paper,
│                            #   each with its own README (thesis, results, code map)
├── figures/                 # Shared figures (papers link to it via symlinks)
├── styles/                  # neurips_2026.sty, shared checklist.tex
├── qrmc_aaai27/             # AAAI-27 submission (self-contained, incl. AAAI kit)
├── qrmc_measurement_framework/  # Full version = Technical Supplement
├── semantic_warp/           # Companion 1: provenance-conditioned completion
├── route_warp/              # Companion 2: route transfer + place identity
├── collective_semantic_memory/  # Companion 3: CSM mechanics
└── symbolic_memory/         # Early combined version (historical)

songline_drive/              # Core library: intents, semantic memory, collective_*.py
experiments/                 # All experiments, one topic per folder, each with README
│                            #   (warp, qrmc_external, big_experiment, llm_collective, ...)
scripts/                     # Benchmarks, oracle interventions, paper figures
tmp/                         # Raw run outputs (registered predictions, rows, verdicts)
```

Papers ↔ experiments mapping lives in `papers/README.md` and in each
paper's README. Experiment code stays importable as `experiments.<name>`.
