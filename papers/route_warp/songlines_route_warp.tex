...
\item \textbf{Meaning-based place identity}: translation-invariant
  ...
  frame at all (\S\ref{sec:stack}). A registered landmark-vocabulary
  ...
  transports in $252$ cells; \S\ref{sec:ablation}). A song-anatomy
...
(\S\ref{sec:route}--\S\ref{sec:rupture} measure what it buys and
...
Four registered predictions, four passes (Table~\ref{tab:ablation}).
\textbf{(A1) The ablation is fail-safe:} in $252$ cells there is not
one fail-open transport --- the fail-closed asymmetry of
\S\ref{sec:identity} holds not just against ambiguous \emph{worlds}
...
\S\ref{sec:identity} opened with a claim we have so far only argued:
...
\item \textbf{Full snapshots} (the machinery of
  \S\ref{sec:identity}): the information upper bound --- same
...
\S\ref{sec:ablation} --- everything that goes wrong goes wrong as a
...
landmark vocabulary $\Sigma$ is hand-fixed; \S\ref{sec:ablation}
...
\S\ref{sec:ablation}) and the song-anatomy experiment (S0.1--S0.4,
\S\ref{sec:anatomy}, bit codec registered with the predictions)
