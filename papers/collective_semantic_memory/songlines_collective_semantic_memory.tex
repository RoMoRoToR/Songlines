...
structure} and asking \emph{when the colimit exists} as a function of the two
protocol parameters --- so that the category computes a structural fact rather
than re-describing an algorithm. Two parameters play two structural roles:
...
\end{description}
A colimit over all $N$ agents --- a consensus object that fuses and serves
every agent's evidence --- exists iff the $R$-round reachability of the
$\tau$-thinned trust graph is all-to-all.
...
$\tau$ the trust graph is dense and the colimit exists at every cadence.
Raising $\tau$ prunes the long-range morphisms down to a near-neighbour ring;
the collective then keeps a colimit \emph{only if} cadence is low enough for
multi-hop composition through still-trusted intermediaries to reconnect it ---
$\tau{=}0.55$ holds the colimit down to $R{=}2$, $\tau{=}0.80$ needs $R\ge4$
...
\begin{figure}[t]
  ...
  $R$ rounds). \textbf{Right:} $P(\text{a colimit over all }N\text{ exists})$,
...
communication diagram, and the diagram's structural invariant (existence of a
colimit) predicts a qualitative regime of the collective. \emph{Scope.} The
trust matrix is produced by the real EMA rule under a modelled ring
co-observation topology, and ``colimit exists'' is operationalised as
