...
\title{Utility-Gated Collective Semantic Memory:\\Formation, Admission, and Long-Horizon Coordination}
...
order-theoretic account (deferred to the appendix) interprets the
merged memory as a coherent gluing of local views whose
consensus-spanning is a measured, trust--cadence-dependent property. The second half studies how records
...
counterfactual replay on held-out worlds (Spearman $0.989$), so at
deployment the runtime requires no online counterfactual replay ---
after calibration on replay-accessible training environments. On long
horizons, unregulated exchange is harmful; temporal decay of foreign
testimony is necessary but insufficient in the evaluated drift setting
--- and a clock-alternatives study shows the operative signal is
receiver-relative (age since own corroboration beats a world clock,
while ordering-only Lamport/vector clocks are worse than no clock);
receiver-side admission --- validating foreign testimony on the
recipient's own visit --- reverses the sign: mean cumulative cost
falls below independent memory on both roles (fragile $118$ vs.\
$148$, robust $134$ vs.\ $171$; ${\approx}20\%$ relative), and a
staged component ladder attributes the reversal to quarantine plus
admission, with reservations adding duplicate hygiene at no cost. An integrated single runtime combines
...
cost; a fixed-budget comparison against five stronger context-management
baselines attributes the freshness gain to structured consolidation as
such (a programmatic latest-state table suffices), while cross-frame
identity remains specific to the graph's alignment machinery. The result is an end-to-end research prototype of utility-gated,
provenance-aware collective memory within the evaluated controlled
substrates; it does not establish open-world
...
non-stationary, resource-scarce multi-agent settings with
heterogeneous roles and potentially stale foreign testimony,
long-term collective memory functions not as a shared archive but as
a distributed formation-and-admission process. Estimated
counterfactual marginal utility governs whether an experience is
retained; structural analogy governs whether it is merged, stored as
an exception, or promoted to a new schema; and foreign records gain
\emph{durable} action authority only after receiver-side admission
under explicit merge, trust, staleness, and coordination constraints
--- while bounded provisional use for transport remains available
under the validity contract (companion route-transfer study). Utility
gating certifies decision usefulness, not truth: retention reflects a
record's measured contribution to task outcome, and the epistemic
burden is carried by the provenance and validity contracts, not by
...
static algebraic description of memory content. Both readings are
interpretations of the implemented system, developed in the
appendices; neither is load-bearing for the empirical claims, and
neither appears among the contributions.
...
removes the shared-frame assumption (formal interpretations deferred
to appendices).
...
hand-designed rule, and a utility estimator that matches exact
counterfactual replay (Spearman $0.989$) and requires no oracle at
deployment (it is calibrated on replay-accessible training worlds).
\item Quarantine and receiver-side admission that turn long-horizon
social exchange from harmful into net-positive (mean cumulative cost
$118$ vs.\ $148$ and $134$ vs.\ $171$ against independent memory,
${\approx}20\%$ relative), resolving the social ladder exchange $\to$
temporal decay $\to$ admission --- with a clock-alternatives study and
a staged component ladder attributing the effect.
...
operations, a learnable controller and a replay-calibrated utility
estimator, the long-horizon social ladder (exchange harmful $\to$
temporal decay necessary-but-insufficient, with the receiver-relative
form strongest among tested clocks $\to$ receiver-side admission),
...
This section restates, for self-containedness, the cadence result
established in the companion Q/R/M/C study \citep{songlines1}; the
sweep and its bottleneck-shift finding are that paper's contribution,
and here they serve as the established starting point that motivates
the CSM design. The peer topology has one explicit parameter:
broadcast cadence $K$. The main sweep crosses four architectures and
eight peer cadences $K{\in}\{1,2,4,8,16,32,48,64\}$ for 35{,}640
runs. Figure~\ref{p4:fig:conditional-rates} summarizes the result.
...
interior optimum of conditional (success-only) time-to-success at $K^\star{=}8$, measured on the
$35{,}640$-run peer-broadcast sweep of the companion Q/R/M/C
study~\citep{songlines1}: $P(M^\star|R^\star)$ falls and
$P(C^\star|M^\star)$ rises with $K$, while mean $t_{\mathrm{succ}}$ has
an interior minimum. (b) The measured CSM sweep over cadence $K$ and
trust threshold $\tau$: mean $t_{\mathrm{succ}}$ is essentially
$\tau$-invariant and only weakly $K$-dependent, and mission success is
flat in $(K,\tau)$ (range $0.015$). The trust threshold is
\emph{not} the active lever in this regime --- the interior operating
point is the cadence effect of panel (a), not a two-dimensional
optimum island.}
\label{p4:fig:ktrust}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Conditional rates and mean time-to-success from the 35{,}640-run sweep. Fast sharing improves target materialization but harms completion through contention; slow sharing has the opposite profile. Among successful episodes, mean time-to-success has a shallow interior minimum at $K{=}8$; under a censoring-aware expected cost the advantage disappears (companion Q/R/M/C study).}
...
full provenance-conditioned analysis appears in the companion route-transfer study \citep{songlines2}; here it serves as the mechanistic reading of
...
the companion route-transfer study \citep{songlines2}.
\begin{enumerate}[leftmargin=20pt,itemsep=2pt,topsep=2pt]
...
These results constructively address the previously stated
shared-frame limitation: place identity can be carried by
...
We formulate memory formation through two conceptually distinct decision dimensions (they are not claimed to be statistically independent). The first
...
The resulting controller selects among five operations. The decision
table below is \emph{formal}: its rows are the mutually exclusive,
exhaustive branches of the implemented rule (utility threshold
$U_{\mathrm{thr}}$; signature-overlap threshold; decision-distortion
threshold $D_{\mathrm{thr}}$), so every incoming record falls into
exactly one row:
\begin{center}
\small
\begin{tabular}{llll}
...
\midrule
high & none (no matching schema) & --- & \textbf{NEW\_SCHEMA} \\
high & complex (overlap below threshold) & --- & \textbf{NEW\_SCHEMA} \\
high & simple & consistent ($D{<}D_{\mathrm{thr}}$) & \textbf{MERGE} \\
high & simple & conflicting ($D{\ge}D_{\mathrm{thr}}$) & \textbf{EXCEPTION} \\
low & none & --- & \textbf{DROP} \\
low & complex & --- & \textbf{DROP} \\
low & simple & consistent & \textbf{REPEAT} \\
low & simple & conflicting & \textbf{REPEAT} \\
\bottomrule
\end{tabular}
\end{center}
Two consequences of the implemented rule are worth making explicit.
The conflict predicate is evaluated only when utility is high: a
conflicting but useless candidate updates statistics (REPEAT) rather
than earning an exception --- EXCEPTION is reserved for evidence worth
acting on. And conflict presupposes a simple analogy (distortion is
measured against a matched schema), so the cell
``complex $\wedge$ conflicting'' is unreachable by construction.
...
\paragraph{Necessity of the operations, by ablation.} Each operation
was removed in turn and the ablated memory run on paired two-world
streams --- an original world A and a corrupted variant V --- on $48$
worlds from seed blocks fully disjoint from every development seed
(registered predictions written before the run; artifact
\texttt{tmp/cluster/operation\_ablations/}). Removing EXCEPTION in
favour of last-write-wins merging destroys the original world
(cost$_A$ $54$--$58$ vs.\ full's $12$--$13$; success$_A$
$1.0 \to 0.0$; corrupt-merge events in every world); removing it in
favour of DROP destroys the corrupted world instead (cost$_V$
$63$--$74$ vs.\ $19$--$20$): the exception cell is exactly what holds
the two-world Pareto point. A binary merge-or-drop policy is
catastrophic on transfer worlds B/C ($100$--$119$ vs.\ $12$--$16$).
Removing REPEAT leaves costs untouched while bloating memory and
erasing support statistics; removing NEW\_SCHEMA leaves cost intact
but corrupts structure --- schemas collapse into a single parent whose
certificate accumulates false failure entries, the damage surfacing in
trust bookkeeping rather than in path cost. Honest accounting of the
registered verdicts: the population-level effects above are large and
stable across all three seed blocks, but the \emph{per-world
universal} form of the exception prediction (strictly lower two-world
max-regret in every single world) fails in $2$ of $48$ worlds, where
last-write-wins happened to carry lower regret ($8$ vs.\ $10$; $5$
vs.\ $8$) --- and one seed block's streams contained too few novel
events to exercise the NEW\_SCHEMA verdict at all. We retain the
universal predictions as registered, report them as $46/48$ and
$2/3$ blocks, and state the necessity claims at the population level.
...
Throughout, \emph{receiver-side admission} means that a foreign
certificate remains quarantined until its utility is evaluated under
the receiver's current role and world version on the receiver's own
visit; only records that pass this local check gain action authority.
...
communication and false-transfer costs. Exact replay is used as an
oracle only during controlled calibration; at runtime the deployed
controller uses the estimator $\widehat U$
(Section~\ref{p5:sec:acceptance}, UE1) rather than any simulator
oracle.

\paragraph{Analogy cost.} A new episode $e$ maps onto a stored schema
$s$ through an analogical map $\psi$ with cost
\[
C_{\mathrm{analog}}(e,s;\psi) = L(\psi) + \lambda_d\,
D_{\mathrm{decision}}(e,s) + \lambda_a \operatorname{Defect}(\psi),
\qquad \mathrm{assim}(e,s) = e^{-C_{\mathrm{analog}}},
\]
...
authority state, and $H$ alignment requirements (what a valid
correspondence would have to satisfy). Here $A$ denotes admission and
authority state; it must not be confused with the temporary analogical
map $\psi_m$ (below), which is reconstructed during consumption and
never stored. Indeed, the concrete analogical map $\psi_m$ and the
inter-frame correspondence $\Gamma_{ij,t}$ are reconstructed by the
receiver at consumption time and are not transmitted as authoritative
state. On exchange, a schema travels with a \textbf{utility
certificate} containing applicability conditions, observed $\Delta V$,
uncertainty, evidence identifiers, alignment requirements $H$, and
recorded failures; the
...
$\Phi$ of the companion Q/R/M/C evaluation study \citep{songlines1}).
...
$\alpha{=}0.05$, $\tau{=}0.30$ (fixed; the trust--staleness constants of the companion route-transfer study \citep{songlines2}); song budget
...
no cost). This is the central empirical pattern of this paper --- \emph{the gate, not the
...
at horizon, exactly as raw exchange was for the LLM collective in the companion route-transfer study \citep{songlines2}.
...
\paragraph{Is a world clock actually necessary? Clock alternatives,
measured.} ``Necessary'' deserved a direct test against the standard
alternatives, and the answer revises our own earlier phrasing
($12$ seeds, $300$ episodes, paired streams; artifact
\texttt{tmp/cluster/song\_grammar/ca/}). Ordering-only logical clocks
do \emph{not} substitute: Lamport ($194.9/258.8$ fragile/robust) and
vector clocks ($195.0/258.9$) are \emph{worse than no clock at all}
($169.8/238.1$) --- they order testimony but never age it out, and the
extra admissible volume amplifies the aliasing failure. Family
version numbers alone ($162.5/220.4$) approximate the world clock
($159.2/211.8$). The strongest temporal gate is \emph{age since the
receiver's own last corroboration} ($129.1/154.5$) --- a purely local
signal requiring no cross-agent time at all, which beats the world
clock and even the independent baseline ($148.2/170.8$) in this
setting, and which is precisely a weak form of receiver-side
admission. The correct statement of S2 is therefore not ``a world
clock is necessary'' but: \emph{temporal decay of foreign testimony
relative to the receiver's own evidence is necessary; a world clock
is one adequate carrier of it, ordering-only clocks are not, and the
receiver-relative form both outperforms global time and anticipates
S3}. All arms here still lose to full admission (S3: $118/134$).
...
\paragraph{The component ladder, measured.} The S1$\to$S3 narrative is
closed by a registered staged ablation on paired assignment streams
($12$ seeds, $300$ episodes; artifact
\texttt{tmp/cluster/song\_grammar/cl/}): each rung adds one component,
and every headline claim of this section re-derives as a rung
difference. Mean cumulative cost (fragile/robust):
...
Exchange hurts (rung 2), staleness gating recovers part (rung 3),
quarantine-without-validation collapses to $\approx$independence
(rung 5, as the theory of admission predicts: held-forever testimony
is inert), admission produces the reversal (rung 6), and reservations
add duplicate hygiene at no cost (rung 7: duplicate rate
$0.015 \to 0.000$). Leave-one-component-out from the full runtime
confirms the attribution and yields one honest negative: removing
admission or quarantine breaks the system back to rung-3/4 costs
($141.4/171.7$ and $139.6/167.8$), removing staleness costs
moderately ($119.6/132.1$), removing reservations returns duplicates
without a cost change --- and removing \emph{provenance tracking}
changes nothing measurable ($118.4/123.8$): in this substrate the
trust weights are carried by admission itself, and provenance's value
(auditability, adversarial attribution) is not visible in the cost
metric. We report the null rather than claim otherwise.

\paragraph{The authority ladder, and why quarantine is not a
deadlock.} A foreign record here passes through four authority states
shared with the companion route-transfer study \citep{songlines2}:
\emph{observed} (the sender's local evidence), \emph{received}
(quarantined: present, version metadata circulating, but inert),
\emph{provisionally executable} (permitted to guide bounded,
reversible transport under an explicit validity-and-coordination
contract), and \emph{admitted} (durable authority after receiver-side
validation). Quarantine restricts the \emph{fourth} state, not the
third. In this paper's long-horizon setting, agents revisit families
in the course of their roles, so quarantined testimony is validated on
visits that occur anyway and strict quarantine suffices; the apparent
circularity --- validation requires a visit, a visit seems to require
acting on the record --- does not arise. Where visits do not recur
naturally, the companion study supplies the missing rung: bounded
provisional transport (staleness gate, reservation, rollback, rupture
boundary) carries the receiver into validation range without granting
durable authority. The two papers describe two rungs of one ladder,
not two competing principles.
...
\paragraph{Beyond rank correlation (UE1, extended).} Because a high
Spearman alone does not certify decision quality, the estimator is
also evaluated on error, calibration, and regret (same frozen
$\lambda$ and thresholds; test seeds $100+$ untouched during any
tuning; artifact \texttt{tmp/song\_grammar/ue1\_eval/}). On the
held-out split: MAE $12.3$ and RMSE $15.9$ against
$\mathrm{std}(U^\star){=}89$ --- so absolute values carry
${\approx}14\%$ error even where ranking is near-perfect; sign
accuracy $0.98$ (precision $0.978$, recall $1.0$ for $U^\star{>}0$);
top-$5/10/20$ recall $0.82/0.88/0.90$; calibration monotone with
weighted error $8.7$, the main defect an over-estimate of weak
records near the retention threshold. The decision-level metric is
\emph{policy regret}: retaining the top-$B$ records by $\widehat U$
instead of by exact replay loses $\le 0.3\%$ of the achievable value
at every tested budget. Out-of-distribution slices: layout-variant
and horizon shifts preserve all metrics (regret $\le 0.7\%$);
the honest weakness is the \emph{role} shift (calibrated on the
robust role, evaluated on fragile-role utilities): magnitudes are
systematically under-predicted (MAE $33.8$, calibration error $30$),
ranking survives ($\rho{=}0.97$, regret $\le 2.3\%$) but top-$5$
recall drops to $0.60$ --- per-role calibration is required where
absolute utility thresholds matter, and we record this as a
limitation rather than average it away. All headline metrics
replicate on a widened split ($30$ pools: MAE $10.7$, $\rho{=}0.991$,
regret $0.2\%$) and on a completely fresh seed block ($200$--$230$,
never touched by any experiment in the program: MAE $10.7$,
$\rho{=}0.990$, sign accuracy $0.976$, regret $0.3\%$).
...
\paragraph{Full resource accounting.} Equal memory and communication
budgets do not by themselves equalize \emph{compute}, so every arm of
the benchmark was re-run under full metering ($30$ test seeds,
$e{=}300$, paired with the utility benchmark; artifact
\texttt{tmp/cluster/song\_grammar/resource\_accounting/}): stored
bytes, transmitted bytes (protocol metadata included), memory-update
time, query latency, process CPU-seconds, and amortized formation
cost per use. The utility-winning arm is not buying its advantage
with hidden compute: the full runtime sits on the joint
(cost $\times$ all six resources) Pareto frontier and even has
\emph{lower} query latency than independent memory ($79$ vs.\
$97$\,ms mean), because consolidation keeps the queried store small.
Its real premiums are storage ($624$\,KB/agent vs.\ $181$ independent)
and wire traffic ($54$\,KB), reported as such. The compute-expensive
arms are the unconsolidated retrieval-scan baselines (query
$345$--$653$\,ms, CPU $1.6$--$2.9\times$ the full runtime), while the
top-3-capped graph baseline is the CPU-cheapest arm of all ($30$
CPU-s) at much worse task cost --- structure is not intrinsically
expensive; scanning is. One metering gap is disclosed: the
learned-formation arm's counterfactual probes made its shards exceed
the walltime ($8$ of $10$ seeds per shard completed; its update cost,
${\sim}870$\,s per seed, is itself the finding), and reservation
traffic in this substrate is estimated, not carried on the wire.
...
advantage is not an artefact of an undertrained baseline. MAPPO
baseline from the companion Q/R/M/C study~\citep{songlines1}.}
...
\paragraph{Evidence classes.} As in the companion studies, results
are marked by causal strength rather than presented uniformly: the
cadence bottleneck shift is the companion study's contribution,
restated as background; CSM dominance and the equal-budget runtime
comparisons are \emph{controlled system comparisons}; the formation
matrix (U1) is a \emph{deterministic replay measurement}; the bandit
and estimator results are \emph{held-out learning comparisons}; the
S1$\to$S2$\to$S3 social ladder is a sequence of \emph{registered
paired interventions}; the LLM active-context result is a
\emph{systems comparison} against raw context and five stronger
context-management baselines (Appendix~\ref{p4:app:ctx-vs-structure},
Axis~4) --- it certifies the value of structured consolidation as
such, with graph-specific value only on cross-frame identity, and is
not evidence of a fundamental long-context limitation; and
the order-theoretic and coalgebraic accounts are \emph{interpretations},
with no empirical weight.
...
The current result is therefore an end-to-end research prototype of
utility-gated, provenance-aware collective memory within the
evaluated controlled substrates. It should not
...
referents (see the companion Q/R/M/C study's limitations \citep{songlines1}), and
...
vocabulary ablation in the companion route-transfer study \citep{songlines2} makes the
...
The colimit reading requires the alignment maps of the diagram; rather
than assume them, Section~\ref{p4:sec:semantic-identity} constructs them,
which gives the colimit reading its scientific function:
\begin{enumerate}[leftmargin=20pt,itemsep=2pt,topsep=2pt]
...
\paragraph{Axis 4: stronger baselines at a fixed token budget.}
The raw transcript is a weak opponent, so we re-ran the freshness and
(single-log) identity questions against five stronger context-management
baselines under one budget accounting (identical model, evidence stream,
questions, and scoring; token counts by one tokenizer; payload $\le$ budget
enforced; artifact \texttt{tmp/llm\_context\_baselines/}; $12$ layouts per
cell, Llama-3.1-8B local): \textbf{raw+instructions} (transcript plus explicit
conflict-resolution guidance, preamble charged to the budget),
\textbf{rolling summary} (incremental LLM summarization; its extra calls are
logged as build cost), \textbf{retrieval} (top-$k$ TF--IDF chunks),
\textbf{table} (a \emph{programmatic} latest-state-per-place table, no LLM,
no graph), and the Songlines \textbf{graph}.

\begin{table}[h]
\caption{Fixed-budget comparison, Llama-3.1-8B, long logs
(${\approx}2{,}000$-token transcripts), freshness / identity accuracy at
representative budgets; ``tok'' = measured payload, ``bc'' = summary build
calls. The uncapped column reproduces the published raw degradation
($0.08$).}
\label{p4:tab:ctx-baselines}
\centering
\small
\begin{tabular}{l cc c cc c cc}
\toprule
 & \multicolumn{2}{c}{uncapped} & & \multicolumn{2}{c}{budget 192} & & \multicolumn{2}{c}{budget 96} \\
Arm & fresh & tok & & fresh & tok & & fresh & tok \\
\midrule
raw            & 0.08 & 1979 & & 0.58 & 182 & & 0.58 & 87 \\
raw+instr      & 0.42 & 2097 & & \textbf{1.00} & 176 & & 0.00 & 96 \\
summary        & 0.83 & 97 (bc 144)  & & 0.92 & 90 & & 0.75 & 70 \\
retrieval      & 0.08 & 2001 & & 0.08 & 176 & & 0.17 & 81 \\
table          & \textbf{1.00} & 76 & & \textbf{1.00} & 76 & & \textbf{1.00} & 76 \\
graph          & \textbf{1.00} & 117 & & \textbf{1.00} & 117 & & 0.33 & 58 \\
\bottomrule
\end{tabular}
\end{table}

Three honest readings. (i) The strongest baseline is the cheapest one: the
programmatic latest-state \emph{table} matches the graph on freshness at
every budget (and on the single-log identity question), at $61$--$76$
tokens. What these two question families certify is therefore the value of
\emph{external structured consolidation as such}, not of the graph
representation specifically. (ii) The failure modes of the other baselines
are informative: TF--IDF retrieval fails freshness at every budget
($0.08$--$0.17$) because lexical similarity preferentially returns the
\emph{stale} sightings; rolling summary is competitive on freshness but
carries a hidden compute cost ($144$ extra LLM calls per log) and loses the
identity question; instructions rescue the raw transcript at moderate
budgets ($1.00$ at $192$) but not at full length ($0.42$ uncapped) --- the
conflict-resolution failure is attentional, not informational. (iii) The
graph's \emph{differential} value over a flat table does not live on these
two axes: it lives on Axis~3, cross-frame identity, where a
latest-state table in one agent's private frame cannot express the merge at
all --- that is the axis that requires the alignment machinery. At the
tightest budget ($96$) the graph serialization is truncated and degrades
($0.33$) while the more compact table survives; representation overhead is
a real cost and is reported as such. The same seven-arm grid on a
second model family (Qwen2.5-3B, cluster GPU, same budgets and
scoring) reproduces the ordering: table and graph at the top of
freshness at every budget, TF--IDF retrieval weakest
($0.42$--$0.58$ on long logs), and the flat table again matching or
beating the graph on the single-log identity question --- the
structured-consolidation attribution is not model-specific.

\paragraph{Summary.}
The same model, given the same evidence, preserves long, drifting, multi-agent
context when the evidence reaches it in consolidated structured form ---
the Songlines graph, or, for the single-frame freshness and identity
questions, even a flat latest-state table (Axis~4): flat-cost prompts,
freshness resolved. Given the raw stream instead, it fails on integration at
$1.3$\,kB, on freshness at $19$\,kB with full ingestion, and on cross-frame
identity at every tested size. The graph-specific value concentrates where
flat structure cannot follow: cross-frame gluing (Axis~3). Scope: two model
families, one task family per axis, and the compute-envelope cells are
reported as such; the graph arm's consolidation here is the deterministic
Songlines rule --- LLM-driven consolidation is the roadmap's next stage.
