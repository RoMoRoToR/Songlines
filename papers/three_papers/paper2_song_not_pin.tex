...
edge-specific rupture law predicts all $110$ tested validity regimes and
all $21$ intermediate failure indices exactly, and a traveler on a
foreign route preserves the witness's measured risk profile up to the
predicted rupture. Semantic landmark constellations recover
...
\par\medskip\noindent\textbf{We claim.} Foreign navigation memory
becomes executable only when referential identity, relational transport,
evidence provenance, temporal validity, and coordination authority are
preserved together. A target without a route is operationally
incomplete; a route without a valid cross-frame correspondence is
unsafe; and foreign evidence without a validity and coordination
contract should not directly determine action. The correct unit of
transferable navigation memory is therefore not a coordinate target but
a provenance-conditioned relational route whose landmarks establish
identity, whose edges provide transport, and whose validity contract
determines how far and under what conditions another agent may execute
it.
\end{abstract}
...
reservation-and-rollback coordination contract. The second
(Sections~\ref{p3:sec:route} onward) shows that the transferable unit is
...
contract defined in the companion evaluation study, warp is an exact
...
$\wstar$ stratum, giving the Q/R/M/C bottleneck shift reported in the companion evaluation study its
...
The route-level analysis contributes the following:
\begin{enumerate}[leftmargin=20pt,itemsep=2pt,topsep=2pt]
...
The trust$\times$staleness inclusion rule defined in the companion
collective-memory study is used throughout with unchanged constants: evidence
mass $\text{trust}\cdot e^{-\alpha\,\text{age}}\cdot m$, threshold
$\tau{=}0.30$, $\alpha{=}0.05$. Warp events follow the provenance
analysis of the first part of this paper: provenance $\varphi$ frozen at lock time; completion attributed
...
warp distance law of Section~\ref{p2:sec:law}, now for paths). The rupture of a transferred route is
...
lasts, and the measured protection ends at the edge predicted by the rupture law.
...
\paragraph{Validity law under recovered frames.} Re-running the warp
distance law of Section~\ref{p2:sec:law} with the trust$\times$staleness gate ON TOP of semantic
...
subject of a companion paper on utility-certified collective memory.
