...
\paragraph{An order-theoretic reading of the Q--R polarity.}
The query--retrieval interface carries standard order-theoretic structure that explains, rather than decorates, the single-agent diagnosis. Let $G$ be the set of place records, $\Sigma$ the tag alphabet, and $I_\theta \subseteq G\times\Sigma$ the incidence ``place $p$ carries tag $\tau$ with confidence at least $\theta$'' induced by the memory's per-place tag tables (Appendix~\ref{app:schemas}). For $B\subseteq\Sigma$ and $A\subseteq G$ put $\mathrm{ext}(B)=\{p : \forall\tau\in B,\ (p,\tau)\in I_\theta\}$ and $\mathrm{int}(A)=\{\tau : \forall p\in A,\ (p,\tau)\in I_\theta\}$. Then
\[
A \subseteq \mathrm{ext}(B) \iff B \subseteq \mathrm{int}(A),
\]
an antitone Galois connection; equivalently, $\mathrm{int}$ and $\mathrm{ext}$ form an adjoint pair between the two power-set posets (one taken with reversed order) viewed as thin categories \citep{ganter1999fca,maclane1998categories}, and the closed pairs form the concept lattice realized by the memory. In this vocabulary the single-agent diagnosis is exact: a query issues an intent $B = q^{\mathrm{req}}$; retrieval fails empty precisely when $\mathrm{ext}(B)=\emptyset$, that is, when the queried meaning lies outside the realized lattice; and consolidation acts on $I_\theta$, so growing the context grows every extent. The 91\%/96.9\% pattern of \S\ref{sec:refined-attribution} says the binding regime is absence of evidence rather than contradiction, which is the regime where the monotone-growth reading applies (Appendix~\ref{app:galois} states the caveat). The multi-agent axes then separate cleanly: memory coupling $\mathcal{M}^{(K)}_{-i}$ merges peer contexts at a rate set by $K$, which can only grow extents, while occupancy $\mathcal{O}_{-i}$ is not part of the context at all; the M$\leftrightarrow$C trade-off of Empirical Claim~1 is a competition between lattice growth (semantic) and resource contention (extra-semantic). The precise statement, its four-line argument, and its deliberate scope limits (graded scores, non-monotone updates, no structure claimed for M or C) are in Appendix~\ref{app:galois}.
...
\item Minimal CSM benchmark (Table~\ref{tab:csm-results}, 3{,}240 runs, $\approx 5$ min):
