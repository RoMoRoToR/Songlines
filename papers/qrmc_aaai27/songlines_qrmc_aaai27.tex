...
\begin{equation}
q=P(Q^\star),\quad
r=P(R^\star\mid Q^\star),\quad
m=P(M^\star\mid R^\star),\quad
c=P(C^\star\mid M^\star).
\end{equation}
...
no-fault diagnosis),
\begin{equation}
G_d=\tfrac{1}{|\mathcal{C}|}\textstyle\sum_{c}\bigl(S_{c,a_d(c)}-S_{c,\mathrm{none}}\bigr),\quad
