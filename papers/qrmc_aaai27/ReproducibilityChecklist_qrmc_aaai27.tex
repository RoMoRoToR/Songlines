...
% DRAFT: событийные определения протокола даны операционально;
% псевдокод логгера — в code supplement, не в теле.
partial
...
% DRAFT: фирменный стиль статьи — non-claims бокс, registered
% predictions, «reading, not theorem».
yes
...
% DRAFT: cognitive maps, goal-conditioned RL, MARL, LLM-agents.
yes
...
% DRAFT (обновлено 2026-07-16 по отзыву): вклад методологический и
% эмпирический; статья сама подчёркивает, что factorization — это
% стандартный chain rule без математической новизны. "no" снимает
% вопрос «где теоретический вклад и почему proofs partial».
% Подпункты ниже — NA.
no
	...
	% DRAFT (обновлено 2026-07-10): в main paper события вложены по
% построению и произведение = тождество (Markov нужен только для
% интерпретации звеньев); формальные statements — Technical
% Supplement, Appendix "Operational vs. nested stage estimators"
% (нотация C*=>M*=>R*=>Q*, оценимость) + Assumption 1. Ответ
% покрывает submission = paper + supplement.
NA

	\question{All novel claims are stated formally (e.g., in theorem statements)}{(yes/partial/no)}
	% DRAFT: Definition 1/2 формальны; Empirical Claim 1 намеренно
% заявлен как эмпирическое slope-level утверждение, не теорема.
NA

	\question{Proofs of all novel claims are included}{(yes/partial/no)}
	% DRAFT: аргументы оценимости в supplement названы arguments,
% не proofs — сознательно.
NA

	\question{Proof sketches or intuitions are given for complex and/or novel results}{(yes/partial/no)}
	% DRAFT: качественное обоснование обеих slope-условий дано.
NA

	\question{Appropriate citations to theoretical tools used are given}{(yes/partial/no)}
	% DRAFT: chain rule, frequency estimation, options/Galois
% (в supplement) с цитатами.
NA

	\question{All theoretical claims are demonstrated empirically to hold}{(yes/partial/no/NA)}
	% DRAFT: стресс-тест Assumption 1 (скрытый конфаундер) +
% верификация обоих наклонов на 35,640 прогонов.
NA

	\question{All experimental code used to eliminate or disprove claims is included}{(yes/no/NA)}
	% DRAFT: NA — теорвклад не заявляется (2.1 = no); сами скрипты
% (assumption1_stress_test.py и др.) всё равно в code supplement.
NA
...
% DRAFT: статических датасетов нет; среды-бенчмарки (MiniGrid,
% BabyAI, VMAS) трактуем как datasets для полноты.
yes
...
\begin{itemize}
	...
	% DRAFT: выбор сред мотивирован (контролируемость, портируемость).
yes

	\question{All novel datasets introduced in this paper are included in a data appendix}{(yes/partial/no/NA)}
	% DRAFT: новых датасетов нет — миры генерируются кодом с
% детерминированными сидами (код в supplement).
NA
...
NA
	...
	% DRAFT: MiniGrid/BabyAI/VMAS процитированы.
yes
	...
	% DRAFT: все среды публичные (Apache/MIT).
yes
...
\begin{itemize}
	...
	% DRAFT: константы CSM (alpha, tau, beta) и K-сетка указаны;
% theta-ablation есть; полные dev-time диапазоны всех параметров
% не документированы.
partial

	\question{Any code required for pre-processing data is included in the appendix}{(yes/partial/no)}
	% DRAFT: препроцессинга как такового нет; генерация сред — в коде.
yes
	...
	% DRAFT: весь код экспериментов и анализа идёт в Code Supplement
% (дедлайн 31 июля).
yes
	...
	% DRAFT: TODO АВТОРУ — подтвердить публикацию репозитория и
% лицензию (в корне репо есть LICENSE).
yes
	...
	% DRAFT: код документирован docstring'ами с отсылками к дизайну,
% но не каждый шаг привязан к номеру раздела статьи.
partial
	...
	% DRAFT: детерминированные сиды всюду; схема stable_rng (md5 от
% ключа) описана; registered-файлы фиксируют предсказания до прогонов.
yes
	...
	% DRAFT: параграф Computing infrastructure добавлен в
% reproducibility-приложение supplement (Apple Silicon, macOS,
% Python 3.9, Ollama, версии библиотек).
yes
	...
	% DRAFT: условные вероятности стадий формально определены;
% мотивация выбора — ядро статьи.
yes

	\question{This paper states the number of algorithm runs used to compute each reported result}{(yes/no)}
	% DRAFT: 35,640 / 3,240 / n=20 на ячейку / 10 сидов — указано везде.
yes
	...
	% DRAFT: bootstrap CI, cluster-robust ресемплинг по 81 ячейке.
yes
	...
	% DRAFT (обновлено 2026-07-10): oracle-эффект — точный sign test
% по парным per-seed дельтам (p=0.004) + bootstrap CI; свип —
% cluster-robust effect sizes + парные cell-bootstrap контрасты
% (K=8 vs соседи/independent, оба временных метрика); external —
% Wilson CI (n=20). Выбор effect sizes вместо голых p-values
% обоснован в тексте.
yes
	...
	% DRAFT: alpha=0.05, tau=0.30, beta=0.10, K-сетка, PPO-гиперпараметры
% в supplement; LLM: temperature 0, модели и бюджеты указаны.
yes
